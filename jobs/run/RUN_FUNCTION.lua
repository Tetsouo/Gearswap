...
local constants = {
    ...
    ABILITY_IDS = {
        ...
        VALLATION = 109,      -- ID for the "Vallation" ability
        VALIANCE = 135,       -- ID for the "Valiance" ability
        PFLUG = 136,          -- ID for the "Pflug" ability
        EMBOLDEN = 138        -- ID for the "Embolden" ability
    },
    SPELL_NAMES = {
...
-- RUN-specific automatic abilities mapping
auto_abilities = {
    -- Uses 'Vallation' ability for defensive spells
    [constants.SPELL_NAMES.PHALANX] = function(spell, eventArgs)
        if try_cast_spell(spell, eventArgs) then
            auto_ability(spell, eventArgs, constants.ABILITY_IDS.VALLATION, 1, 'Vallation')
        end
    end,
    
    -- Uses 'Embolden' for Foil to boost accuracy
    [constants.SPELL_NAMES.FOIL] = function(spell, eventArgs)
        if try_cast_spell(spell, eventArgs) then
            auto_ability(spell, eventArgs, constants.ABILITY_IDS.EMBOLDEN, 1, 'Embolden')
        end
    end
}
