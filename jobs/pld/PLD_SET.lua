...
sets.precast.JA['Holy Circle'] = set_combine(sets.FullEnmity, { feet = createEquipment('Rev. Leggings +3') })
...
sets.precast.WS['Savage Blade'] = set_combine(sets.precast.WS, {
    ...
    neck = createEquipment("knight's bead Necklace +2"),
    waist = createEquipment("Sailfi Belt +1"),
    left_ear = createEquipment("thrud Earring"),
    right_ear = createEquipment("Tuisto Earring"),
...
sets.engaged = set_combine(sets.idleNormal, {
    ...
    head = createEquipment("Sakpata's helm", 0),         -- HP+114
    body = createEquipment("Sakpata's breastplate", 13), -- HP+228
    hands = createEquipment("Sakpata's Gauntlets", 11),  -- HP+91
    legs = createEquipment('Chev. Cuisses +3', 11),      -- HP+127
    feet = createEquipment("Flamma Gambieras +2", 0),    -- HP+68
    neck = createEquipment('Lissome necklace', 12),      -- Pas de bonus HP
    waist = createEquipment('Sailfi Belt +1', 6),        -- Pas de bonus HP
    left_ear = createEquipment('Cessance Earring', 0),   -- Pas de bonus HP
    right_ear = createEquipment('Dedition Earring', 4),  -- Pas de bonus HP
...
sets.engaged.AFM3 = set_combine(sets.idleNormal, {
    ammo = createEquipment('Aurgelmir Orb +1', 2),           -- Pas de bonus HP
    head = createEquipment("Chevalier's Armet +3", 0),       -- HP+114
    body = createEquipment('Flamma korazin +2', 13),         -- HP+228
    hands = createEquipment("Chevalier's Gauntlets +3", 11), -- HP+91
    legs = createEquipment('Chev. Cuisses +3', 11),          -- HP+127
    feet = createEquipment("Flamma Gambieras +2", 0),        -- HP+68
    neck = createEquipment('Null Loop', 12),                 -- Pas de bonus HP
    waist = createEquipment('Kentarch Belt +1', 6),          -- Pas de bonus HP
    left_ear = createEquipment('Telos Earring', 0),          -- Pas de bonus HP
    right_ear = createEquipment('Dominance Earring +1', 4),  -- Pas de bonus HP
    left_ring = Moonlight1,
