...
local UIConfig = ConfigLoader.load_ui_config('Gabvanstronger', 'BLM')
...
function get_sets()
    ...
    _G.BLMTPCONFIG = require('Gabvanstronger/config/blm/BLM_TP_CONFIG')
...
function user_setup()
    ...
    local BLMStates = require('Gabvanstronger/config/blm/BLM_STATES')
    BLMStates.configure()

    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Gabvanstronger/config/blm/BLM_KEYBINDS')
