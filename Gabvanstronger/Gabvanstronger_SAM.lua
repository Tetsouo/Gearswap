...
local UIConfig = ConfigLoader.load_ui_config('Gabvanstronger', 'SAM')
...
function get_sets()
    ...
    _G.SAMTPConfig = require('Gabvanstronger/config/sam/SAM_TP_CONFIG')
...
function user_setup()
    ...
    local SAMStates = require('Gabvanstronger/config/sam/SAM_STATES')
    SAMStates.configure()

    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Gabvanstronger/config/sam/SAM_KEYBINDS')
