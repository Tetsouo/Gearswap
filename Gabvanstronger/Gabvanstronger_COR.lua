...
local UIConfig = ConfigLoader.load_ui_config('Gabvanstronger', 'COR')
...
function get_sets()
    ...
    _G.CORTPCONFIG = require('Gabvanstronger/config/cor/COR_TP_CONFIG')
...
function user_setup()
    ...
    local CORStates = require('Gabvanstronger/config/cor/COR_STATES')
    CORStates.configure()

    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Gabvanstronger/config/cor/COR_KEYBINDS')
