...
local movement_threshold = config.get_movement_threshold and config.get_movement_threshold() or 0.5  -- Plus sensible au mouvement
local check_interval = config.get_movement_check_interval and config.get_movement_check_interval() or 1  -- Check chaque frame pour réactivité maximale
    ...
    if mov.counter > check_interval then
        ...
        if pl and pl.x and pl.y and pl.z and mov.x then
            ...
            if dist > movement_threshold then
                ...
                ----------------------------------------------------------------------------
            elseif dist < movement_threshold then  -- Suppression du stable_counter pour déséquiper instantanément
                if moving then
