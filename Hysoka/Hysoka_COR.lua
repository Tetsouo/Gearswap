...
local UIConfig = ConfigLoader.load_ui_config('Hysoka', 'COR')
...
function get_sets()
    ...
    _G.CORTPCONFIG = require('Hysoka/config/cor/COR_TP_CONFIG')
...
function user_setup()
    ...
    local CORStates = require('Hysoka/config/cor/COR_STATES')
    CORStates.configure()

    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Hysoka/config/cor/COR_KEYBINDS')
