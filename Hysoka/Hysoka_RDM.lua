...
local UIConfig = ConfigLoader.load_ui_config('Hysoka', 'RDM')
...
function get_sets()
    ...
    _G.RDMTPCONFIG = require('Hysoka/config/rdm/RDM_TP_CONFIG')

    -- Load RDM Saboteur configuration
    local saboteur_config_success, RDMSaboteurConfig = pcall(require, 'Hysoka/config/rdm/RDM_SABOTEUR_CONFIG')
...
function user_setup()
    ...
    local RDMStates = require('Hysoka/config/rdm/RDM_STATES')
    RDMStates.configure()
    ...
    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Hysoka/config/rdm/RDM_KEYBINDS')
