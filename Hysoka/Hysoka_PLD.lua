...
local UIConfig = ConfigLoader.load_ui_config('Hysoka', 'PLD')
...
function get_sets()
    ...
    _G.PLDTPCONFIG = require('Hysoka/config/pld/PLD_TP_CONFIG')
    _G.BluMagicConfig = require('Hysoka/config/pld/PLD_BLU_MAGIC')
...
function user_setup()
    ...
    local PLDStates = require('Hysoka/config/pld/PLD_STATES')
    PLDStates.configure()
    ...
    if is_initial_setup then
        -- Load keybinds
        local success, keybinds = pcall(require, 'Hysoka/config/pld/PLD_KEYBINDS')
