...
local UIConfig = ConfigLoader.load_ui_config('Hysoka', 'DRK')
...
function get_sets()
    ...
    _G.DRKTPCONFIG = require('Hysoka/config/drk/DRK_TP_CONFIG')
...
function user_setup()
    ...
    local DRKStates = require('Hysoka/config/drk/DRK_STATES')
    DRKStates.configure()
    ...
    if is_initial_setup then
        -- Load keybinds
        local success, keybinds = pcall(require, 'Hysoka/config/drk/DRK_KEYBINDS')
