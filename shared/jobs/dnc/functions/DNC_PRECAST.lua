...
function job_precast(spell, action, spellMap, eventArgs)
    ...
    if spell.type == 'WeaponSkill' and WeaponSkillManager then
        ...
        if WS_DB[spell.english] then
            ...
            -- Display WS description only (TP displayed by TPBonusHandler after gear equip)
...
function job_post_precast(spell, action, spellMap, eventArgs)
    if spell.type == 'WeaponSkill' then
        ...
        -- SECOND: Apply TP bonus gear and display final TP
        if TPBonusHandler then
            TPBonusHandler.apply_and_display(spell, DNCTPConfig)
        end
    end
end
