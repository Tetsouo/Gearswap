...
-- RUN logic modules
local AOEManager = require('shared/jobs/run/functions/logic/aoe_manager')
local RuneManager = require('shared/jobs/run/functions/logic/rune_manager')
...
function job_self_command(cmdParams, eventArgs)
    ...
    if CommonCommands.is_common_command(command) then
        ...
        if CommonCommands.handle_command(command, 'RUN', table.unpack(args)) then
            eventArgs.handled = true
        end
        return
    end
    ...
    if command == 'debugmidcast' then
        ...
        add_to_chat(159, '[RUN_COMMANDS] Debug toggled! Current state: ' .. tostring(_G.MidcastManagerDebugState))
