...
local UIConfig = ConfigLoader.load_ui_config('Morphetrix', 'RDM')
...
function get_sets()
    ...
    _G.RDMTPCONFIG = require('Morphetrix/config/rdm/RDM_TP_CONFIG')

    -- Load RDM Saboteur configuration
    local saboteur_config_success, RDMSaboteurConfig = pcall(require, 'Morphetrix/config/rdm/RDM_SABOTEUR_CONFIG')
...
function user_setup()
    ...
    local RDMStates = require('Morphetrix/config/rdm/RDM_STATES')
    RDMStates.configure()
    ...
    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Morphetrix/config/rdm/RDM_KEYBINDS')
