...
local UIConfig = ConfigLoader.load_ui_config('Kaories', 'PLD')
...
function get_sets()
    ...
    _G.PLDTPCONFIG = require('Kaories/config/pld/PLD_TP_CONFIG')
    _G.BluMagicConfig = require('Kaories/config/pld/PLD_BLU_MAGIC')
...
function user_setup()
    ...
    local PLDStates = require('Kaories/config/pld/PLD_STATES')
    PLDStates.configure()
    ...
    if is_initial_setup then
        -- Load keybinds
        local success, keybinds = pcall(require, 'Kaories/config/pld/PLD_KEYBINDS')
