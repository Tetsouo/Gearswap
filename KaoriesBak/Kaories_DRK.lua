...
local UIConfig = ConfigLoader.load_ui_config('Kaories', 'DRK')
...
function get_sets()
    ...
    _G.DRKTPCONFIG = require('Kaories/config/drk/DRK_TP_CONFIG')
...
function user_setup()
    ...
    local DRKStates = require('Kaories/config/drk/DRK_STATES')
    DRKStates.configure()
    ...
    if is_initial_setup then
        -- Load keybinds
        local success, keybinds = pcall(require, 'Kaories/config/drk/DRK_KEYBINDS')
