...
local function load_settings()
    local success, settings = pcall(function()
        local success_ret, result = pcall(require, 'config/settings')
        if not success_ret then
            error("Failed to load config/settings: " .. tostring(result))
        end
        return result
    end)
...
---============================================================================
--- MACRO BOOK MANAGEMENT HELPERS
---============================================================================

--- Get macro book configuration for dual-boxing scenario.
--- Returns the macro book and page for a specific main job + sub job + alt job combination.
---
--- @param main_job string The main character's job (e.g., 'THF', 'WAR')
--- @param sub_job string The main character's subjob (e.g., 'WAR', 'NIN')
--- @param alt_job string The alt character's job (e.g., 'GEO', 'COR')
--- @return table|nil Macro configuration with book and page, or nil if not found
function config.get_dual_box_macro(main_job, sub_job, alt_job)
    if config.settings.macros.dual_box[main_job] and
        config.settings.macros.dual_box[main_job][sub_job] then
        return config.settings.macros.dual_box[main_job][sub_job][alt_job]
    end
    return nil
end

--- Get solo macro book configuration.
--- Returns the macro book and page for a specific main job + subjob combination.
---
--- @param main_job string The main job (e.g., 'THF', 'WAR')
--- @param sub_job string The subjob (e.g., 'WAR', 'NIN')
--- @return table|nil Macro configuration with book and page, or nil if not found
function config.get_solo_macro(main_job, sub_job)
    if config.settings.macros.solo[main_job] then
        return config.settings.macros.solo[main_job][sub_job]
    end
    return nil
end

--- Get lockstyle set number for a job.
--- Returns the configured lockstyle set number for a specific job.
---
--- @param job string The job abbreviation (e.g., 'THF', 'WAR')
--- @return number|nil The lockstyle set number, or nil if not configured
function config.get_lockstyle(job)
    return config.settings.macros.lockstyles[job]
end

--- Get the complete macro configuration.
--- Returns the entire macro configuration for advanced usage.
---
--- @return table The complete macro configuration
function config.get_macro_config()
    return config.settings.macros or {}
end
