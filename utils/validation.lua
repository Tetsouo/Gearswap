...
-- Create a local log replacement since logger might not work in FFXI
local log = {
    error = function(msg, ...)
        local formatted = string.format(msg, ...)
        if windower and windower.add_to_chat then
            windower.add_to_chat(167, "[GearSwap][ERROR] " .. formatted)
        end
    end,
    warn = function(msg, ...)
        local formatted = string.format(msg, ...)
        if windower and windower.add_to_chat then
            windower.add_to_chat(057, "[GearSwap][WARN] " .. formatted)
        end
    end,
    debug = function(msg, ...)
        -- Silent debug
