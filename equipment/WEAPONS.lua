...
function WeaponUtils.check_weaponset(weaponType)
    ...
    elseif player.main_job == 'BST' then
        ...
        -- BRD-specific weapon handling
    elseif player.main_job == 'BRD' then
        return WeaponUtils.handle_brd_weapons(weaponType)
...
--- Handles BRD-specific weapon set logic
--- @param weaponType (string): 'main' or 'sub'
--- @return (boolean): Success status
function WeaponUtils.handle_brd_weapons(weaponType)
    if not (state and state.MainWeapon) then
        log.error("BRD weapon state variables not properly initialized")
        return false
    end
    ...
    if weaponType == 'main' then
        if not sets[state.MainWeapon.current] then
            log.error("BRD main weapon set does not exist: %s", state.MainWeapon.current)
            return false
        end

        success, error_msg = pcall(equip, sets[state.MainWeapon.current])
        if not success then
            log.error("Failed to equip BRD main weapon: %s", error_msg or "unknown error")
            return false
        end
    elseif weaponType == 'sub' then
        -- BRD sub weapon logic based on subjob (like other jobs)
        log.debug("BRD sub weapon check - subjob: %s", player.sub_job or "none")

        if player.sub_job == 'DNC' or player.sub_job == 'NIN' then
            -- Use dual wield off-hand
            log.debug("BRD using DualWield set for subjob %s", player.sub_job)
            if sets.DualWield then
                log.debug("BRD DualWield set found: %s", sets.DualWield.sub or "no sub defined")
                success, error_msg = pcall(equip, sets.DualWield)
                if not success then
                    log.error("Failed to equip BRD sub weapon: %s", error_msg or "unknown error")
                    return false
                end
            else
                log.error("BRD DualWield set not found")
            end
        else
            -- Use shield for other subjobs
            log.debug("BRD using Shield set for subjob %s", player.sub_job)
            if sets.Shield then
                log.debug("BRD Shield set found: %s", sets.Shield.sub or "no sub defined")
                success, error_msg = pcall(equip, sets.Shield)
                if not success then
                    log.error("Failed to equip BRD sub weapon: %s", error_msg or "unknown error")
                    return false
                end
            else
                log.error("BRD Shield set not found")
            end
        end
    end
