...
function get_sets()
    ...
    _G.PLDTPCONFIG = require('Typioni/config/pld/PLD_TP_CONFIG')
    ...
    -- Load AutoMove first so PLD_MOVEMENT can register callbacks
...
function user_setup()
    ...
    local PLDStates = require('Typioni/config/pld/PLD_STATES')
    PLDStates.configure()
    ...
    if is_initial_setup then
        -- Load keybinds
        local success, keybinds = pcall(require, 'Typioni/config/pld/PLD_KEYBINDS')
