...
if not success or not UIConfig then
    ...
    add_to_chat(167, "[DNC] UIConfig load failed, using defaults")
else
end
...
function get_sets()
    ...
    _G.DNCTPConfig = require('Typioni/config/dnc/DNC_TP_CONFIG')
    _G.DNCWSConfig = require('Typioni/config/dnc/DNC_WS_CONFIG')
    ...
    -- Load AutoMove first so DNC_MOVEMENT can register callbacks
...
function user_setup()
    ...
    local DNCStates = require('Typioni/config/dnc/DNC_STATES')
    DNCStates.configure()
    ...
    if is_initial_setup then
        ...
        local success, keybinds = pcall(require, 'Typioni/config/dnc/DNC_KEYBINDS')
