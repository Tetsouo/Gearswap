...
function RDMStates.configure()
    ...
    state.SubWeapon =
        ...
        'Genmei Shield' -- DT shield
    }
    state.SubWeapon:set('Ammurapi')
    ...
    state.NukeMode =
        ...
        'LowTierNuke', -- Low-tier nukes (MP conservation)
        'Accuracy' -- Magic Accuracy focus
    }
    ...
    state.RefreshMode =
        ...
        'Off', -- Manual refresh
        'On' -- Auto-refresh party members
    }
    state.RefreshMode:set('Off')
...
function RDMStates.validate()
    ...
    if not state.RefreshMode then
        return false, 'RefreshMode not configured'
    end
