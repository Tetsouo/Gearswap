...
if not success or not UIConfig then
    ...
    add_to_chat(167, "[BLM] UIConfig load failed, using defaults")
else
end
...
function get_sets()
    ...
    _G.BLMTPCONFIG = require('Typioni/config/blm/BLM_TP_CONFIG')
    ...
    -- Load AutoMove first so BLM_MOVEMENT can register callbacks
...
function user_setup()
    ...
    local BLMStates = require('Typioni/config/blm/BLM_STATES')
    BLMStates.configure()

    if is_initial_setup then
        -- KEYBIND LOADING
        local success, keybinds = pcall(require, 'Typioni/config/blm/BLM_KEYBINDS')
